...
function(rna,rna2mod,modList,net){
	...
	anno_row <- data.frame(Module = rep(paste0("M",1:mNum), sapply(modules,length)))
	row.names(anno_row) <- moduleProt1
	# dev.new(width=8, height=8)
	pheatmap(adjMat,annotation_row=anno_row,cluster_rows=FALSE,cluster_cols=FALSE,show_colnames=FALSE,
			color=colorRampPalette(c("white",color1[4]))(5),main=title,legend=FALSE)
}
