...
function(rna2prot,modlist,pCutoff = 0.01,bgProtNum){
	...
	uniRNA = names(which(sort(table(rna2prot[,1]),decreasing=T)>1))
	...
	for (i in 1:rnaNum){
		...
		for (j in 1:moduleNum){ 
			...
			RNA2moduleMat[i,j]=1-phyper(hitNum-1,moduleSize,bgProtNum-moduleSize,targetNum)
		}
	}

	# return a mapping matrix with rows represent ncRNAs while columns represent modules
